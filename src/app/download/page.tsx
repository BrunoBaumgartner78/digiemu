    ...
    <div className="min-h-screen px-4 py-8 md:px-8">
      <div className="max-w-4xl mx-auto">
        <header className="section-header mb-6">
          ...
          <p className="text-sm text-muted">Hier findest du alle digitalen Produkte, die du über DigiEmu gekauft hast.</p>
        </header>
        ...
        {orders.length === 0 ? (
          ...
          <div className="space-y-4">
            {orders.map((order) => {
              ...
              const expired = hasDownload && downloadLink!.expiresAt < new Date();

              return (
                <div key={order.id} className="neumorph-card p-4 md:p-5 flex flex-col md:flex-row gap-4 md:items-center">
                  <div className="flex-1">
                    <h2 className="font-semibold text-base md:text-lg">{product?.title ?? "Produkt"}</h2>
                    <p className="text-xs text-muted mb-1">
                      Bestellt am {order.createdAt.toLocaleDateString("de-CH", { day: "2-digit", month: "2-digit", year: "numeric" })}
                    </p>
                    {product?.category && <p className="text-xs text-muted">Kategorie: {product.category}</p>}
                  </div>
                  ...
                  <div className="flex flex-col items-start md:items-end gap-2">
                    {hasDownload ? (
                      expired ? (
                        ...
                        <Link href={`/download/${order.id}`} className="neobtn-sm primary">🔽 Download</Link>
                      )
                    ) : (
                    ...
                    {product?.id && (
                      <Link href={`/product/${product.id}`} className="nav-link text-xs">Produktseite ansehen</Link>
                    )}
                  </div>
                </div>
              );
