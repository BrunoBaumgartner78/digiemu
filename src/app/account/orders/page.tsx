        ...
        <table className="min-w-full bg-[#232323] rounded-lg">
          ...
          <tbody>
            {orders.map((o) => {
              ...
              if (o.downloadLink) {
                const expired = new Date(o.downloadLink.expiresAt) < new Date();
                if (expired) {
                  ...
                  downloadLink = (
                    <Link
                      href={`/download/${o.id}`}
                      className="px-2 py-1 bg-[#39FF14] text-[#1A1A1A] rounded font-bold"
                    >
                      Download
                    </Link>
                  );
              ...
              return (
                <tr key={o.id} className="border-t border-gray-700 text-white">
                  ...
                  <td className="px-4 py-2">
                    {downloadLink || <span className="text-gray-400">{downloadStatus}</span>}
                  </td>
                </tr>
              );
            })}
