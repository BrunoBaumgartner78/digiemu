...
export default function EditProductForm(props: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch("/api/vendor/profile-status");
        if (!res.ok) {
        ...
        <section className={styles.card}>
          ...
          <div className={styles.toggleRow}>
            <button
              ...
              disabled={isBlocked || canPublish === false}
              title={canPublish === false ? "Warte auf Freigabe durch Admin" : undefined}
            >
              Aktiv
            </button>
            <button
          ...
          {canPublish === false && (
            <div className="text-xs text-[var(--text-muted)] mt-2">Veröffentlichen ist erst nach Admin-Freigabe möglich.</div>
          )}
